...
// @desc    Generate flashcards and quiz from document content
...
// @desc    Generate summary from document content
...
export const chat = async (req, res, next) => {
  try {
    ...
    if (!documentId || !question) {
      return res.status(400).json({
        success: false,
        message: "Please provide a document ID and a question",
        statusCode: 400,
      });
    }
...
export const explainConcept = async (req, res, next) => {
  try {
    ...
    if (!documentId || !concept) {
      return res.status(400).json({
        success: false,
        message: "Please provide a document ID and a concept to explain",
        statusCode: 400,
      });
    }
  ...
  try {
    ...
    if (!chatHistory) {
      return res.status(200).json({
        success: true,
        data: [], // No chat history yet, return empty array
