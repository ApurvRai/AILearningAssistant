  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white/80 backdrop-blur-xl border-2 border-slate-200 rounded-2xl shadow-xl shadow-slate-200/50 p-6 mb-8">
        <div
          className="inline-flex items-center gap-2 px-4 py-2 bg-linear-to-r from-emerald-50 to-teal-50 border
                        border-emerald-200 rounded-xl mb-6"
        >
                ...
                className={`group relative flex items-center p-3 border-2 rounded-xl
                          cursor-pointer transition-all duration-200 ${
                            isSelected
                              ...
                              : "border-slate-200 bg-slate-50/50 hover:shadow-md hover:border-slate-300 hover:bg-white"
                          }`}
              >
                <input
                  type="radio"
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        {currentQuestionIndex === quiz.questions.length - 1 ? (
          <button
            className="group relative px-8 h-12 bg-linear-to-r from-emerald-500 to-teal-500 rounded-xl
                         hover:from-emerald-600 hover:to-teal-600 text-white font-semibold text-sm transition-all duration-200
                         shadow-lg shadow-emerald-500/25 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:active-scale-100 overflow-hidden"
          ...
          <Button disabled={submitting} onClick={handleNextQuestion}>
            Next
            <ChevronRight
              className="w-4 h-4 group-hover:translate-x-0.5 transition-transform duration-200"
              strokeWidth={2.5}
      ...
      <div className="mt-0 flex items-center justify-center gap-2 flex-wrap">
        {quiz.questions.map((_, index) => {
          ...
          return (
            <button
              className={`w-8 h-8 font-semibold rounded-lg text-xs transition-all duration-200 ${
                isCurrent
