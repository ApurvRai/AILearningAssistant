...
const QuizManager = ({ documentId }) => {
  ...
  const handleGenerateQuiz = async (e) => {
    ...
    setGenerating(true);
    try {
      const data = await aiService.generateQuiz(documentId, { numQuestions });
        ...
        <form onSubmit={handleGenerateQuiz} className="space-y-4">
          <div>
            ...
            <input
              ...
              min="1"
              required
              className="w-full h-9 px-3 border border-neutral-200 rounded-lg bg-white text-sm text-neutral-900 focus:border-transparent
                        placeholder-neutral-400 transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-[#00d492]"
            />
          </div>
