  ...
  return (
    <>
      <div
        ...
        duration-300 ${isSidebarOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`}
        onClick={toggleSidebar}
        aria-hidden="true"
      ></div>
      <aside
        className={`fixed top-0 left-0 h-full w-64 bg-white/90 backdrop-blur-lg border-r
        border-slate-200/60 z-50 md:relative md:w-64 md:shrink-0 md:flex md:flex-col md:translate-x-0
        transition-transform ease-in-out duration-300 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        ...
        <div className="px-3 py-4 border-t border-slate-200/60">
          <button
            ...
            className="group flex items-center gap-3 w-full py-2.5 px-4 text-sm font-semibold rounded-xl
            transition-all duration-200 text-slate-700 hover:text-slate-900 hover:bg-red-50 hover:text-red-600"
          >
            <LogOut
              size={18}
              strokeWidth={2.5}
